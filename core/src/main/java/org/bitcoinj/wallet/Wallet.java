...
public class Wallet extends BaseTaggableObject
    ...
    private FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        while (true) {
            ...
            if (req.useInstantSend && feePerKb.compareTo((params.isDIP0001ActiveAtTip() ? TransactionLockRequest.MIN_FEE.div(10) : TransactionLockRequest.MIN_FEE).multiply(tx.getInputs().size())) < 0) {
                feePerKb = (params.isDIP0001ActiveAtTip() ? TransactionLockRequest.MIN_FEE.div(10) : TransactionLockRequest.MIN_FEE).multiply(tx.getInputs().size());
            }
            else if (needAtLeastReferenceFee && feePerKb.compareTo(params.isDIP0001ActiveAtTip() ? Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.div(10) : Transaction.REFERENCE_DEFAULT_MIN_TX_FEE) < 0) {
                feePerKb = params.isDIP0001ActiveAtTip() ? Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.div(10) : Transaction.REFERENCE_DEFAULT_MIN_TX_FEE;
            }
            Coin feeNeeded = req.useInstantSend ? feePerKb : feePerKb.multiply(size).divide(1000);
    ...
    public void setTransactionBroadcaster(@Nullable org.bitcoinj.core.TransactionBroadcaster broadcaster) {
        ...
        for (Transaction tx : toBroadcast) {
            ...
            //Added for Dash due to incorrect fees and other issues that may cause crashing
            if(confidenceType == ConfidenceType.UNKNOWN)
            {
                log.error("A pending transaction of type UNKNOWN {}\nNot Broadcasting", tx.toString());
                continue;
